<!DOCTYPE html>
<html>
    <head>
        <meta charset="utf-8" />
        <link rel="stylesheet" href="css/globals.css" />
        <link rel="stylesheet" href="css/styleguide.css" />
        <link rel="stylesheet" href="css/style.css" />
    </head>
    <body>
        <div class="box">
            <!-- rectangle vert -->
            <div class="rectangle03">
                <header class="header">
                    <div class="box_header">
                        <div class="logo"></div>
                        <nav>

                            <!-- retour a l'accueil -->
                            <ul>
                                <li class="back_users_home_page">Accueil</li>
                            </ul>

                            <!-- lien metier -->
                            <ul>
                                <li class="job_item">Photographe</li>
                                <li class="job_item">infographiste PAO</li>
                                <li class="job_item">Intégrateur web</li>
                                <li class="job_item">Monteur vidéo</li>
                            </ul>

                            <!-- Lien pour etre -->
                            <ul>
                                <li class="to_be">Devenir infographiste</li>
                                <li class="to_be">Publier une annonce</li>
                            </ul>

                            <!-- icon se connecter -->
                        </nav>


                        <div class="user-menu-container">
                            <div class="user-icon" id="userIcon">
                                <div class="circle-user"></div>
                            </div>
                            <div class="menu" id="menu">
                                <ul>
                                    <li><a href="connexion.html">Se connecter</a></li>
                                </ul>
                            </div>
                        </div>


                    </div>
                </header>
            </div>   
            <!-- end rectangle vert -->
        </div>

        <main>
            <div class="rectangle-div">
                <div class="title_infographiste">
                    Motion designer
                </div>
                <!-- juste un design -->
                <div class="infographiste_summ">
                    <div class="rectangle-div01">
                        <div class="profil_picture01" style="background-image: url(media/public/images/photos_membres/phto_say1.png);">
                            <!-- <img class="phto-say2-icon" alt="" src=""> -->
                        </div>

                        <div class="box_summ">
                            <div class="origin_x_name">
                                <div class="name">Say Christ Emmanuel</div>
                                <div class="origin">Côte d'ivoire, Songon</div>
                            </div>
    
                            <!-- <div class="title_work_mode">Mode de travail</div>
                            <div class="work_mode">Distance</div> -->

                            <div class="title_specia">specialité</div>
                            <div class="specia">
                                <span>MONTEUR VIDEO</span>
                            </div>

                            <div class="title_contract">Contrat</div>
                            <div class="contract">
                                <span>CDD</span>
                                <span>STAGE</span>
                                <span>CDI</span>
                            </div>
                            
                            <div class="title_diplome">Diplome</div>
                            <div class="diplome">
                                <ul>
                                    <li>Licence professionnelle Arts et images numérique</li>
                                </ul>
                            </div>
                            
                            <!-- <div class="title_social_links">Liens social</div> -->
                            <!-- <div class="social_link">
                                <ul>
                                    <li><img src="behance.svg" width="18" alt=""> <a href="#">www.behance.com</a></li>
                                    <li><img src="behance.svg" width="18" alt=""> <a href="#">www.behance.com</a></li>
                                    <li><img src="behance.svg" width="18" alt=""> <a href="#">www.behance.com</a></li>
                                </ul>
                            </div> -->

                        </div>
                    </div>

                </div>   


                <div class="boxmindset_infographiste">
                    <h4 class="mindset">Mindset</h4>
                    <p class="text_mindset">
                        "Neque porro quisquam est qui dolorem ipsum quia dolor sit amet, consectetur, adipisci velit...
"Neque porro quisquam est qui dolorem ipsum quia dolor sit amet, consectetur, adipisci velit...
                    </p>

                    <h4 class="portfolio">Portfolio</h4>
                    <div class="box_illustation_created">
                        <div class="card_illustration_created illustration_01">
                            <p class="illustration_description">
                                Spot animatique et motion <br> logo 2d / 3d
                            </p>
                        </div>
                        <div class="card_illustration_created illustration_01">
                            <p class="illustration_description">
                                Conception de storyboard
                            </p>
                        </div>
                        <div class="card_illustration_created illustration_01">
                            <p class="illustration_description">
                                Montage vidéo
                            </p>
                        </div>
                        <div class="card_illustration_created illustration_01">
                            <p class="illustration_description">
                                Charte graphique
                            </p>
                        </div>
                    </div>


                    <div class="remark">
                        <p class="text_remark color_text_remark">
                            NB : L'absence d'une gestion des transactions financieres
entre vous et le professionnel. nous oblige a nous désengager de toutes relations entre vous et le professionnel
                        </p>
                    </div>

                    <div class="box__contact_profil_wha">
                        <div class="numero"><a href="tel:[+225]0545475763"><i></i>Appeler</a></div>
                        <span>OU</span>
                        <div class="numWha"><a href="https://wa.me/2250545475763"> what's app : 05 XX XX XX XX</a></div> 
                        <!-- <span>OU</span> -->
                        <!-- <div class="numWha"><a href="/pdf/#"> voir CV</a></div>  -->
                    </div>

                </div>
            </div>

        </main>


        <!-- <script src="script_hovers.js"></script> -->
    </body>
    </html>